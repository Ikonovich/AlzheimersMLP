...
class YouChooseDataset(InMemoryDataset):

    def download(self):
        pass

    def __init__(self, transform=None, pre_transform=None):
        root = "C:\\Users\\evanh\\Documents\\Datasets\\YouChoose\\"
        super(YouChooseDataset, self).__init__(root, transform, pre_transform)
    ...
    def process(self):
        ...
        buys = open('DataHandling\\Datasets\\YouChoose\\raw\\yoochoose-buys.dat', 'r')
        clicks = open('DataHandling\\Datasets\\YouChoose\\raw\\yoochoose-clicks.dat', 'r')

        click_df = pd.read_csv(clicks, header=None)
        click_df.columns = ['session_id', 'timestamp', 'item_id', 'category']

        buy_df = pd.read_csv(buys, header=None)
