...
def forward_propagate(network,input):
    ...
    for l, layer in enumerate(network):
        ...
        z = calculate(layer,layer_input)
        if l == len(network) - 1:
            layer_input = sigmoid(z) # output layer activation function
        else:
            layer_input = relu(z) # hidden layer activation function

        layer['outputs'] = layer_input

    return layer_input

def back_propagate(network,expected,lr):
    errors = None
    for l, layer in enumerate(reversed(network)):
        ...
        derivative = relu_derivative(outputs)
...
def train(network,X_train: np.ndarray,y_train: np.ndarray,learning_rate):
    ...
    for s in tqdm(range(len(X_train))):
        ...
        back_propagate(network,expected,learning_rate)
...
def test(network,X_test: np.ndarray,y_test: np.ndarray):
    ...
    for s in tqdm(range(len(X_test))):
        ...
        output_num = to_int(output.flatten())
...
if __name__ == '__main__':
    ...
    # network = initialize_network(n_inputs,n_hidden_layers,n_hidden_nodes,n_outputs)
